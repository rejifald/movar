...
![code health: 81 (B)](https://img.shields.io/badge/code_health-81_%28B%29-green)
...
- **Code health** `81 (B)` — fallow maintainability score, 0-100 with an A-F grade, over complexity, duplication, dead code, and churn; refreshed by `pnpm metrics`.
...
- **Lines of code** `63.1k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
- **Suppressions** `47 eslint-disable · 23 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site.
