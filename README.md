...
- **Lines of code** `44.3k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
- **Suppressions** `43 eslint-disable · 23 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site.
