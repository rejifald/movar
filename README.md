...
![coverage: 92% lines · 85% branches](https://img.shields.io/badge/coverage-92%25_lines_%C2%B7_85%25_branches-green)
...
- **Coverage** `92% lines · 85% branches` — Vitest (v8) line and branch coverage, aggregated across the test-bearing workspace projects weighted by size; snapshotted by `pnpm metrics`.
...
- **Lines of code** `27.8k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
- **Suppressions** `34 eslint-disable · 25 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site.

<sub>Generated by `pnpm gen:readme` from committed sources; the code-health score and coverage are snapshotted by `pnpm metrics`. The badges at the top of the README flag the self-explanatory metrics; this list is the full legend.</sub>
