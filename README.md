...
![code health: 79 (B)](https://img.shields.io/badge/code_health-79_%28B%29-green)
...
- **Code health** `79 (B)` — fallow maintainability score, 0-100 with an A-F grade, over complexity, duplication, dead code, and churn; refreshed by `pnpm metrics`.
...
- **Lines of code** `71.7k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
- **Suppressions** `49 eslint-disable · 22 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site.
