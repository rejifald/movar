...
![content.js: 29 KB](https://img.shields.io/badge/content.js-29_KB-blue)
...
- **Content.js** `29 KB` — minified size of the always-on content script, injected into every page — franc and the language profiles live in the background worker, not here; held under budget by `check:content-bundle` and snapshotted by `pnpm metrics`.
...
- **Lines of code** `28.7k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
