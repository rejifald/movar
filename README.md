...
- **Promises** `4/4 kept` — public claims from movar.fyi (apps/marketing) verified against the code each build — `pnpm check:readme` fails if any breaks:
  ...
  - ✓ **Nothing leaves your browser** — manifest declares data collection 'none', no analytics dependency, and no fetch/XHR/WebSocket/sendBeacon in the extension runtime _(marketing: hero badge + OG card + privacy section)_
...
- **Lines of code** `69.5k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
