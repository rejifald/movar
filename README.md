...
![coverage: 70% lines · 67% branches](https://img.shields.io/badge/coverage-70%25_lines_%C2%B7_67%25_branches-yellow)
...
- **Coverage** `70% lines · 67% branches` — Vitest (v8) line and branch coverage, aggregated across the test-bearing workspace projects weighted by size; snapshotted by `pnpm metrics`.
...
- **Lines of code** `27.4k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
- **Suppressions** `32 eslint-disable · 32 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site.
