...
- **Lines of code** `27.6k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
- **Suppressions** `32 eslint-disable · 25 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site.
