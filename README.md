...
![code health: 84 (B)](https://img.shields.io/badge/code_health-84_%28B%29-green)
...
- **Code health** `84 (B)` — fallow maintainability score, 0-100 with an A-F grade, over complexity, duplication, dead code, and churn; refreshed by `pnpm metrics`.
...
- **Lines of code** `30.7k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
