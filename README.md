...
- **Lines of code** `47.7k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
- **Suppressions** `45 eslint-disable · 23 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site.
