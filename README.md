...
- **Lines of code** `~61k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics` (or `pnpm gen:readme --refresh-source`) and held within ±0.5k of a live scan by the metrics gate — rounded to the nearest thousand because that is the precision the gate guarantees.
- **Suppressions** `46 eslint-disable · 23 fallow-ignore` — inline lint / complexity opt-outs across the tree; the fallow-ignore set is budgeted and each is justified at the call site. Exact counts — the metrics gate fails a PR whose committed numbers differ from a live scan.
