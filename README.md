...
![coverage: 97% lines · 90% branches](https://img.shields.io/badge/coverage-97%25_lines_%C2%B7_90%25_branches-brightgreen)
...
- **Coverage** `97% lines · 90% branches` — Vitest (v8) line and branch coverage, aggregated across the test-bearing workspace projects weighted by size; snapshotted by `pnpm metrics`.
...
- **Lines of code** `60.3k` — TypeScript under `apps/` + `packages/`, tests excluded; snapshotted by `pnpm metrics`.
