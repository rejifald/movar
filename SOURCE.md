...
- **`declarativeNetRequest` uses a single dynamic rule.** At runtime the
  extension installs one rule, driven by the user's settings, that rewrites the
  outgoing `Accept-Language` request header on navigations
  (`browser.declarativeNetRequest.updateDynamicRules`). The source of truth is
  [`apps/extension/src/lib/dnr.ts`](apps/extension/src/lib/dnr.ts); the
  user-facing wording is the `declarativeNetRequest` permission line in the root
  README. The extension never inspects or modifies request or response bodies.
- **Manifest source of truth.** The Firefox manifest — including the
  `browser_specific_settings.gecko` block (the add-on `id`, the minimum-Firefox
  floors for desktop and Android, and
  `data_collection_permissions: { required: ['none'] }`) — is generated by WXT
  from [`apps/extension/wxt.config.ts`](apps/extension/wxt.config.ts). Read the
  current floors there rather than from this doc; they are pinned to the Firefox
  versions where `data_collection_permissions` shipped.
