...
| `src/offline/picker.visual.spec.ts`          | Pixel baselines for the two language-picker surfaces that structural/geometry specs cannot see: the in-row chip a `list` picker gets (`picker-listbox-ru`) and the control badge a `native` `<select>` gets (`picker-select-uk`), at rest and hovered to its label — × light/dark (6 baselines). The `inline` layout's survivor tooltip is pinned separately by `tooltip.visual.spec.ts`                                           |
