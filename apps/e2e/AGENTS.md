...
| `src/offline/picker.visual.spec.ts`                     | Pixel baselines for the other two picker surfaces: the in-row chip a `list` picker gets (`picker-listbox-ru`) and the control badge a `native` `<select>` gets (`picker-select-uk`), the badge both at rest and hovered to its label — × light/dark (6 baselines)                                                                                                                                                                                                                                     |
...
| `src/offline/picker-badge.behavior.spec.ts`             | Badge geometry against a control flush with the right viewport edge: beside the control and never over it, hovered label stays inside the viewport, a reposition while hovered survives the collapse, the badge follows a control a late banner pushes down, scroll tracking holds, and an SPA re-render leaves no tooltip host behind                                                                                                                                                                |
  ...
  src/
    fixtures/
      ...
      html/                     # static HTML fixtures for offline specs
        ...
        picker-select-uk.html   # <select> picker, uk page — control-badge visual baseline (light + dark)
