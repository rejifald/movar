...
/*
 * A draft is withheld from four places at once — the index, the feed, the
 * sitemap and the robots meta — while staying reachable at its own URL so it
 * can be circulated for review. Three of those read the `draft` flag through
 * `isPublished`; the sitemap reads the frontmatter directly in
 * `astro.config.mjs`.
 *
 * Any one of them regressing quietly publishes something unfinished, and none
 * of them would fail a build doing it. `DRAFT_POST` names whichever post is
 * currently under review — when the flag is cleared, this suite is what should
 * be updated to point at the next one, or deleted with the last.
 */
const DRAFT_POST = '/uk/blog/movu-rakhuyut';

test.describe('blog — drafts are reachable but unannounced', () => {
  test('the draft renders at its own URL', async ({ page }) => {
    const response = await page.goto(DRAFT_POST, { waitUntil: 'domcontentloaded' });

    expect(response?.status(), 'a draft must still be served — the link is the point').toBe(200);
    await expect(page.locator('article h1')).toBeVisible();
  });

  test('but tells search engines to stay away', async ({ page }) => {
    await page.goto(DRAFT_POST, { waitUntil: 'domcontentloaded' });
  ...
  test('and is absent from the index, the feed and the sitemap', async ({ page, request }) => {
    await page.goto(INDEX, { waitUntil: 'domcontentloaded' });
    await expect(
      page.locator(`a[href*="${DRAFT_POST}"]`),
      'a listed draft is simply published',
    ).toHaveCount(0);
    // The index still lists the posts that are published, so this is a filter
    // rather than an empty page.
    await expect(page.locator(`a[href*="${POST}"]`)).not.toHaveCount(0);
