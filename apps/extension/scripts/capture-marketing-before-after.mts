...
async function capture(cap: Capture): Promise<void> {
  ...
  try {
    ...
    await page.waitForLoadState('networkidle', { timeout: 10_000 }).catch(() => {
      // Networkidle is best-effort here — Google's SERP frequently has
      // long-poll connections that never reach idle. The 10s timeout is
      // a "give the page a fair chance, then move on" budget; failing
      // it doesn't fail the capture.
    });
