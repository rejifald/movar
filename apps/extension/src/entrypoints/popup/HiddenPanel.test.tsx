...
describe('HiddenPanel', () => {
  ...
  it('lists hidden picker languages, collapsed pickers and feed cards (plural forms)', () => {
    render(
      <HiddenPanel
        hidden={summary({ languages: ['ru', 'be'], containers: 2, feedCards: 3 })}
        onRestore={vi.fn()}
      />,
    );
    ...
    expect(screen.getByText(t.feedHidden(3))).toBeTruthy();
  ...
  it('omits the picker-languages line when only feed cards are hidden', () => {
    render(<HiddenPanel hidden={summary({ feedCards: 1 })} onRestore={vi.fn()} />);
