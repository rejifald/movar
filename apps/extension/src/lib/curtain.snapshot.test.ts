...
describe('curtain structural snapshots — chip skin', () => {
  it('cover + chip + light', () => {
    ...
    expect(serializeCurtain(host)).toMatchInlineSnapshot(`
      ...
      shadow: <style>[styles len=10893]</style><div class="curtain"><button class="chip" type="button" aria-label="Мовар приховав цей блок."><span class="chip__icon" aria-hidden="true">⚑</span><span class="chip__label">Прихований вміст</span></button></div>"
    `);
  });
  ...
  it('replace + chip + light', () => {
    ...
    expect(serializeCurtain(host)).toMatchInlineSnapshot(`
      ...
      shadow: <style>[styles len=10893]</style><div class="curtain"><button class="chip" type="button" aria-label="Мовар приховав цей перемикач мов."><span class="chip__icon" aria-hidden="true">⚑</span><span class="chip__label">Українська</span></button></div>"
    `);
  });
