...
The same seven scenes also ship to the **Safari extension's App Store listing** in portrait, at Apple's fixed device sizes — **iPhone 6.9″ 1320×2868** and **iPad 13″ 2048×2732** (App Store rejects off-spec dimensions). The **macOS** App Store no longer reuses this set: it shows the macOS app itself at 2560×1600, captured from the built app by `pnpm --filter @movar/extension capture:macos-screenshots` into `screenshots/macos/{uk,en}/`. That runs a UI-test target (`safari/Movar/ScreenshotsUITests/`) because neither `screencapture -R` nor `-l<windowid>` can photograph another app's window from an automation context — the first silently returns wallpaper at the right dimensions. Only iOS/iPad need the portrait re-layout.

**Device-tier rule (enforced).** Every screenshot surface renders the fake website in the layout that matches its device — **phone screenshots show the phone layout, tablet the tablet layout, desktop the desktop layout**. It is not declared per scene: each frame derives the tier from its canvas width (`storyboards/device-tiers.ts`) and stamps `movar-device-{phone,tablet,desktop}` on the scaled-content wrapper, and every backdrop carries tier-scoped CSS (`.movar-device-phone .movar-backdrop-*`, …) that selects the matching layout — desktop is the base, phone/tablet are overrides. New scenes inherit this automatically; `storyboards/device-tiers.test.tsx` guards the width→tier map and the frame's class stamp. **When you add or edit a backdrop, give it phone + tablet layouts** — author the phone layout at ~520px and the tablet at ~1024px composition width so it scales to a dense phone / @2x iPad render, not a blown-up desktop page.
...
- **Scenes #8 and #9 — actual host-app UI (`08-host-app-about.png`, `09-host-app-setup.png`)**: unlike the seven extension-in-Safari scenes above, these are full-bleed captures of the **real** native host app — About (brand lede, version, licence, support and legal links) and Settings with the "One last step" banner (Settings ▸ Apps ▸ Safari ▸ Extensions ▸ Movar), the visible Guideline 4.2 evidence. They were one scene while About was a React tab carrying both; going native split them. They come from the app in a **simulator**, not from the React bundle — nothing displays that bundle any more, so a shot of it depicts a screen no user or reviewer can reach (Guideline 2.3.3). Capture: `pnpm --filter @movar/extension capture:host-app-screenshots --prepare --device=<ios|ipad> --locale=<uk|en>`, navigate to the screen, then the same command with `--scene=<08-host-app-about|09-host-app-setup>`. `simctl` has no tap primitive, so navigation is manual; the script boots, reinstalls (which resets the banner's dismissal flag), launches in a locale, grabs native device pixels, refuses an off-spec raster, and flattens alpha. Sizes: 1320×2868 (`iPhone 17 Pro Max`) and 2064×2752 (`iPad Pro 13-inch (M5)`; 2048×2732 is equally valid for the 13″ slot). See [`README.md`](./README.md) for the simulator-app bootstrap and the iPad status-bar language caveat. iPad renders the same screens as a centred 660pt column (`movarColumnMeasure`), so both device sets show the app measured rather than stretched.
