...
- **Scene #8 — actual host-app UI (`08-host-app-about.png`)**: unlike the seven extension-in-Safari scenes above, this is a full-bleed capture of the **real** host app (the `@movar/safari-host-app` About tab — brand lede, "What Movar does", and the Settings ▸ Apps ▸ Safari ▸ Extensions ▸ Movar enable path), the visible Guideline 4.2 evidence. It can't come from the extension's Storybook (the host app owns `html`/`body` with fixed bars + `100dvh`), so it has its own capture: `pnpm --filter @movar/safari-host-app build:bundle` then `pnpm --filter @movar/extension capture:host-app-screenshots` — which loads the built bundle at 440×956@3× (iOS → 1320×2868) and 1024×1366@2× (iPad → 2048×2732) and writes `screenshots/{ios,ipad}/{en,uk}/08-host-app-about.png`. iOS-only recommended for the listing: on iPad the phone-first layout renders as a centered 600px column (authentic but airy).
