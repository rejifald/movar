...
- **No literal third-party logos.** The search-rewrite scene reuses `google-serp-frame.tsx` from the marketing diptych — that frame is editorial illustration (approximated coloured wordmark) rather than the trademarked mark; the same approximation appears in `apps/marketing/src/components/BeforeAfter.astro`.
