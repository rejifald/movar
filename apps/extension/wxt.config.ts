...
export default defineConfig({
  ...
  manifest: ({ browser }) => ({
    ...
    action: {
      // Movar's neutral resting icon (a stone ring, no state badge) — NOT the
      // plain brand mark (`icon/16.png`, kept for the `icons` set + store +
      // Safari). A tab we haven't resolved a state onto still shows a border,
      // so it never reads as border-less. Generated by scripts/generate-icons.mts.
      default_icon: {
