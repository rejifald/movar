...
description: 'Мовне налаштування — це число в чужій таблиці, і рядка «не визначився» в ній немає. Ось ці числа, їхні джерела і те, що сталося, коли люди змінили налаштування.'
...
залежала від того, що показала статистика.
...
навпаки. Розробники застосунків і власники сайтів не здогадуються, якою мовою ви
читаєте: вони це знають. Щось ви повідомляєте їм самі, щось вони бачать із того,
що ви відкрили. І майже одразу ваша мова перетворюється на число в чужій
...
таблиці немає — як немає й рядка «не визначився». Обидва потрапляють в один
...
Valve каже це про власні рішення, а не за видавців ігор. Але таблиця відкрита, а
іншої такої просто немає — більше ніде не подивитися, скільки людей у світі
запускають ігри якою мовою. Тож коли хтось прикидає, чи окупиться українська
локалізація, дивитися він буде сюди.
...
не зобовʼязаний — ваша програма не обовʼязково потрапить у вибірку цього місяця.
...
Сайт перекладають туди, звідки приходять покупці, і ця арифметика в київській
...
Саме тому 47% — це не вирок аудиторії, а показ того, як довго голосує одне давнє
...
Каталог ріс не тільки тому, що зʼявлялося більше перекладів, а й тому, що сам
каталог ставав повнішим. І ще одне, що варто знати. За підрахунками аналітичної
служби GameSensor, **частка** платних ігор з українською досягла піку у
2018–2019 роках і відтоді зменшується, хоча самих таких ігор більшає. Ігор
виходить більше, ніж встигають перекладати.
...
2024 році й відтоді сповзає вниз. Почасти це «падіння» — через те, що річний
...
Wikipedia в цілому почала меншати. Але звузили його не законом і не квотою —
його звузили ті, хто клацнув інше посилання. Тут навіть налаштувань не було:
лише сума окремих переходів.
...
Межа цих трьох чисел велика, і саме на них обіцянка з початку статті не
виконується: Serpstat не оприлюднила ні набору даних, ні періоду збору, ні
розподілу за мовою запиту. Перерахувати їх ви не зможете. Вони лишаються тут
тому, що іншого заміру цього механізму немає, — і спиратися на них варто як на
єдиний, а не як на перевірений.
...
доріжок не переслуховували, тож нижче стоять номери серій — перевірити можна за
чверть години.
...
По-перше, більший корпус не означає кращу відповідь: два абзаци вище показують,
чим саме він більший — 193 сайти, 3,6 млн статей за рік, 33% відповідей
...
змін. Один рядок у цій сумі другого знака після коми не зрушить: одного
користувача серед понад сотні мільйонів на графіку не видно. І те, що за
наступні майже три роки додалося лише 0,06, — теж сума. Сума тих, хто цього ще
не зробив.
...
ним. Її додають тоді, коли бачать, що є кого рахувати. А в ті таблиці, крім нас,
потрапити нікому — і ми в них уже є. Питання лише в тому, у якому стовпчику.
