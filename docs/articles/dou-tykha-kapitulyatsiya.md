...
description: 'Я прибрав російську з усіх налаштувань, а інтернет все одно спілкується зі мною російською. Розбір, чому так стається, і що з цим робити.'
author: Oleksandr Zhuravlov
date: '2026-08-01'
target: dou.ua
status: published
url: https://dou.ua/forums/topic/61178
superseded-by: apps/marketing/src/content/blog/tykha-kapitulyatsiya.md
---

> **Це архівна копія — саме той текст, що пішов на dou.ua.** Живий примірник
> статті тепер живе на сайті:
> [`apps/marketing/src/content/blog/tykha-kapitulyatsiya.md`](../../apps/marketing/src/content/blog/tykha-kapitulyatsiya.md)
> (публікується на `/uk/blog/tykha-kapitulyatsiya`). Правки змісту — там; цей
> файл лишається незмінним записом того, що було подано, тому дві копії
> розходяться навмисно. Ілюстрації спільні: обидві посилаються на
> `apps/marketing/src/content/blog/assets/`.

# Тиха капітуляція: як інтернет змушує відступати від рідної мови

_Я прибрав російську з усіх налаштувань, а інтернет все одно спілкується зі мною російською. Розбір, чому так стається, і що з цим робити._
...
англійська. Навіть перевіряв, як браузер відправляє заголовок `Accept-Language`, і
...
Але вебсайти та пошук вперто спілкуються зі мною російською. Я використав усі
...
![До і після Мовара: той самий сайт і домен, але замість російської версії відкривається українська](../../apps/marketing/src/content/blog/assets/before-after-tochka24.png)
...
Немає однієї причини — це комплект факторів і механізмів, які в підсумку дають
простий результат: контент не тією мовою, яку я обрав.

**Мова браузера — лише підказка.** Браузер відправляє заголовок `Accept-Language`
з кожним запитом — це прохання, а не команда. Багато серверів на нього взагалі не
дивляться. Обробка цього заголовка залежить лише від розробника конкретного
вебсайту.
...
**Кешування.** Сторінка з некоректною мовою живе в кеші й не оновлюється.
...
оголошує пошуковикам, що за цією адресою — українська версія. І ось що ключове: машинерія, яка вирішує, яку версію показати в пошуку
й як її підписати, переважно довіряє тому, що сайт про себе декларує — атрибуту
`lang`, `hreflang`, структурі URL, sitemap, — а не тому, що насправді в тексті.
Ніхто не звіряє одне з одним. Тож російська сторінка потрапляє в індекс як
українська; ви фільтруєте видачу за українською — і отримуєте російські результати
...
російською написано 3,4% сайтів проти 0,7% українською, — тож
неоднозначний кириличний текст відкочується до більшого корпусу. Самі детектори до
...
маркетплейси, дошки оголошень і все, де є користувацький вміст. Інтерфейс там буває
бездоганно українським, а в перемикачі мов російської немає навіть у списку —
обирати можна хіба між українською та англійською. Але картки товарів, описи,
характеристики й відгуки — російською, бо мову вмісту не перевіряє ніхто або не
звертає на це уваги. Тут навіть немає куди перемикатися. Сторінка справді
українська — російські в ній лише окремі блоки.
...
![Стовпчикова діаграма мов клієнтів Steam за липень 2026: російська третя з 9,30%, українська п'ятнадцята з 0,70%, одразу попереду італійської з 0,63%](../../apps/marketing/src/content/blog/assets/steam-languages.png)
...
![Схема шести сигналів визначення мови сторінки, від найнадійнішого до найслабшого: перемикач мов, атрибут html lang, піддомен, сегмент шляху, self-hreflang, текст сторінки](../../apps/marketing/src/content/blog/assets/signal-ladder.png)
...
під рукою, браузери вже вбудовують перекладачі, і вже доволі добре. Мовар цього не
робить принципово, і це, мабуть, найважливіше рішення в усьому продукті.
...
відфільтрованим вмістом: приховати чи закрити шторкою.

![Попап Мовара на сторінці новинного сайту: мова сторінки українська, фільтр вмісту ввімкнено, обрано режим «Лишати за завісою», нижче кнопки паузи на годину та назавжди](../../apps/marketing/src/content/blog/assets/popup-settings.png)
...
кінця означало б показати вам нуль результатів замість тих, що таки є, і врешті
відмовитись від використання розширення.
...
- **Баг-репорти.** Сайт, де Мовар не спрацював — або спрацював там, де не мав, —
  найцінніше, що можна надіслати. Кожен такий випадок стає тестом і більше не
  повторюється.
...
Встановити, а також прочитати повний розбір усіх цих механізмів — на
[movar.fyi](https://movar.fyi/uk) (сторінка [«Чому так стається»](https://movar.fyi/uk/why-this-happens));
код — на [GitHub](https://github.com/rejifald/movar).
...
але еволюціонував в окремий пакет. Він дивиться не лише на сам текст, а й на
...
_Disclaimer: я автор проєкту Мовар. Це некомерційний проєкт, ліцензія MIT. Буду
радий зворотному звʼязку, особливо критичному._
