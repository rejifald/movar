...
7. **Package boundaries.** `@movar/lang-detect` keeps pure detection + oracle (incl. `DivergenceKind` in `shadow.ts`). The view-model types (`DiagCard` / `DiagPicker` / `PageDiagnostics`) live in `apps/diagnostics` (`src/types.ts`); product settings/policy now live in the focused `@movar/brand` / `@movar/settings` / `@movar/events` packages, and the old `DetectionDivergence` / `DiagnosticsSummary` record types were dropped rather than relocated. The reused models are now **proper workspace packages** — `@movar/page-content` (extractor) + `@movar/lang-pickers` (model) + `@movar/page-language` + `@movar/page-mode` — consumed as ordinary `@movar/*` deps. (The original interim was a dev-only `@product` source alias into `apps/extension`; it was removed once the models became packages, so nx now sees the dependency edge correctly.) Only the _pure_ model files are reachable (no `conceal`/`curtain`/`tooltip`/`i18n` rendering); that boundary is machine-enforced — see decision 10.

8. **The reuse boundary is machine-enforced, not just documented.** A contract test (`apps/diagnostics/src/lib/page-diagnostics.purity.test.ts`) statically walks the value-import graph from the real reuse entry (`page-diagnostics.ts`) across resolved `@movar/*` package sources and asserts it touches ONLY the pure model packages + `@movar/lang-detect`(+`/franc`) + `@movar/settings`/`@movar/host-match`, and **never** a `conceal`/`curtain`/`tooltip`/`i18n` module or any `apps/extension/**` file. It shares its forbidden-pattern list and walker with the extension's `capability-boundary.test.ts` + `model-purity.test.ts` and the `modelPurity` ESLint preset, so the "pure models only" rule can no longer drift silently.
9. **Excluded from release + publish.** [`release.yml`](../.github/workflows/release.yml) builds only `@movar/extension`; the dev extension is never zipped, signed, or submitted. No store assets, no i18n (English-only).
10. **Privacy invariant holds trivially.** Everything is local: the dev extension records on-device, shows in an on-device in-page panel, sends nothing. It is also never in a user's hands.
...
```
 Tab (content script, isolated world)
   ┌──────────────────────────────────────────────────────────────────┐
   │  @movar/page-content model (buildModelForHost) → cards              │
   │  @movar/lang-pickers model (findLanguagePickers + buildPickerModel) │
...
`page-diagnostics.ts` runs the reused product models, classifies their output, builds the snapshot, and owns the highlight map + a current-snapshot store. The panel (`Panel.tsx`) renders it. **There is no intra-extension message protocol** — the UI calls `getCurrent()` / `highlightNode(id)` directly and re-renders via the store's `subscribe` hook. (Style isolation: the UI mounts in a shadow root, and design tokens are scoped to `:host` since `:root` doesn't match inside a shadow tree. The reused models are ordinary `@movar/*` package deps — pure model files only, never the product's rendering; the `page-diagnostics.purity.test.ts` contract test enforces that.)
...
- **The model-reuse boundary was a soft boundary; it is now hard.** Originally diagnostics imported `apps/extension` source via a dev-only `@product` alias — a soft coupling nx couldn't see and nothing enforced (a stray `conceal`/`curtain`/`i18n` import would have ridden along). That alias is gone: the models are now `@movar/*` workspace packages (nx sees the edge), and the `page-diagnostics.purity.test.ts` contract test statically asserts the reuse graph touches only pure model files (decision 10). Diagnostics still couples to the models' shapes — if those move, it breaks at typecheck/build (acceptable; it _wants_ to track them).
