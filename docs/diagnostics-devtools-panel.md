...
7. **Package boundaries.** `@movar/lang-detect` keeps pure detection + oracle (incl. `DivergenceKind` in `shadow.ts`). The view-model types (`DiagCard` / `DiagPicker` / `PageDiagnostics`) live in `apps/diagnostics` (`src/types.ts`); product settings/policy now live in the focused `@movar/brand` / `@movar/settings` / `@movar/events` packages, and the old `DetectionDivergence` / `DiagnosticsSummary` record types were dropped rather than relocated. The reused models (`page-content/` extractor + `lang-pickers/` model) are imported from the product source via a **dev-only `@product` alias** (`wxt.config.ts` / `tsconfig.json` / `vitest.config.ts`) — only the _pure_ model files (no `conceal`/`curtain`/`tooltip`/`i18n` rendering). The clean long-term path is promoting those models into a shared package both apps depend on; the alias is the pragmatic interim that leaves the trust-critical product untouched (and avoids dragging `lang-codes`, used widely across the product, into a package right now).
