...
| Stale-listing handling    | **DOM.RIA is the model** — a buyer complaint gives the seller 7 days to respond or the listing auto-deletes; a second complaint removes it outright; sellers are prompted to "Підтвердити актуальність" with auto-archive. OLX report reasons include a not-current/sold option. AUTO.RIA expires after 30 days (90-day cap). Shafa appears rule-violation only. |
