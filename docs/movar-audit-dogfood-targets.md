...
| `core/serving-cookie-overrides-header`                                                                        | Needs a warm cookie leg (`--warm`)         | **Closed since this pass.** The CLI had no flag, so every run read `not-applicable` ("every probe was cold"). `--warm` now collects a warm leg beside the cold matrix; this batch predates it and still reads cold. |
