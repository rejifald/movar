...
- The **general-purpose** tier of the same built-in-AI stack (Prompt API / Gemini Nano, and its `Summarizer` / `Writer` / `Rewriter` / `Proofreader` siblings) is refused separately, and on different grounds — measurement rather than ethics — in [no-llm-language-detection.md](./no-llm-language-detection.md). Note the overlap: a **summary** of blocked content would run into this ADR's laundering argument as well as that one's, since a fluent Ukrainian précis of Russian content re-presents it exactly as a translation would.
