...
- **`ua/state-language-version-lesser` counts _versions_, never collected pages.** A page count is a property of the crawl, not of the site: `/` copied from `/en/` is one version enumerated at two paths, and one URL observed from two vantages is one version observed twice. Counting pages read `en: 2, uk: 1` on a build with exact 1:1 parity and stamped Law 2704-VIII on it. `versionsOf` groups the page set by normalized location and by the hreflang alternates the pages themselves declare — `x-default` included, since there it declares cluster membership rather than a language — and each version counts at most once per language. A version whose own hreflang names a Ukrainian counterpart the run never collected is credited to Ukrainian instead of counted against the site: that absence is a fact about the budget or the sitemap cap, and probing the href to settle it is the one thing this pack may not do. The credit is asymmetric on purpose — evidence _against_ a named company must be observed, evidence _for_ it may be declared — and narrower than `declaresUkrainianVersion` on purpose too, since a picker option is site-wide chrome that would credit every version on any site with a language menu. Merging on declarations can only join versions, never split them, so sloppy hreflang costs count coverage rather than manufacturing a deficit.
