...
- **A page rule's `pass` carries the pages behind it.** The same refusal, one level down: the kernel puts `pagesAdjudicated` / `pagesNotApplicable` on every page-scoped `RuleResult` and sums them into `coverage.pageAdjudications` / `pageNotApplicable`, so a rule that judged one page of three cannot be read as having looked at the site. The verdict is unchanged — a rule that judged a page and found it fine did `pass` — but the report is no longer silent about the other two.
...
- **`pagesAdjudicated` is not `pageAdjudications`.** The near-identical names are deliberate, because the units differ: `RuleResult.pagesAdjudicated` counts **distinct pages** for one rule, while `CoverageSummary.pageAdjudications` counts **(rule, page) pairs** summed over every page rule that ran, so it happily exceeds the page count. Both are optional and absent means "not available", never `0`: a site rule iterates no pages and a `not-collected` rule was never called, and a zero there would read as a judgement the kernel never made. Related: `notApplicableReason` is the **first** of `notApplicableReasons`, kept for a one-line render; the list is the complete set of distinct reasons, in first-seen order. A page rule that skips three pages for three different reasons used to report the first as though it were the rule's.
