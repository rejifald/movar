...
- **`normalizeBCP47` is not a well-formedness oracle.** It returns `null` for any tag outside Movar's alias set (`de-DE` → `null`), which is right for "should Movar act on this?" and wrong for "is this malformed?". Use `isWellFormedBCP47` for the grammar and `declaredLanguageOf` for the comparison.
