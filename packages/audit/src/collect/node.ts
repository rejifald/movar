...
import { createProber, parseRobots, robotsAllows } from './probe';
...
export async function collectNetwork(options: NetworkCollectOptions): Promise<Evidence> {
  ...
  const seenPageUrls = new Map<string, string>();
  ...
  const digestInto = (
    ...
    const existing = seenPageUrls.get(url);
    if (existing !== undefined) return existing;
    const id = `page-${pages.length + 1}`;
    seenPageUrls.set(url, id);
