...
- **Cheap heuristic** (`detectCyrillicLanguage`, `isRussian`): single-pass regex count over distinctive Cyrillic letters (і/ї/є/ґ → uk; ы/ё → ru; ў → be; dense ъ → bg), then a contradiction check that withdraws a call the text argues with (Belarusian without a `ў` is not Ukrainian, however many `і` it has). Returns a `DetectionResult` with `language` and `ukScore`/`ruScore` — the tallies survive a withdrawal. Used where performance matters most (content-script hot path).
- **Snippet classifier** (`classifyBySnippet`): four-rung ladder — (1) candidate-set-relative distinctive alphabet chars, (2a) function words, (2b) corpus-frequent words, (3) franc trigram backstop — over which sits one veto: a winner whose own alphabet cannot account for 2% of the text's letters (`CONTRADICTION_SHARE`) loses to `'unknown'`, and the runner-up is not promoted. Returns a `SnippetVerdict` with `language`, `margin`, and the deciding `rung`. Distinctiveness is always computed at runtime against the passed candidate set; nothing is pre-differenced.
- **Tier-7 engine orchestrator** (`detectLanguageFromText`, `ENGINES`): walks `[chromeAiEngine, francEngine]` in order, returns the first non-null result. chrome-ai (the browser's on-device LanguageDetector, Chrome 138+) is opportunistic — never triggers a download; falls through to franc (always available, ~170 KB, 187 languages).
...
- `classifyBySnippet` returns `'unknown'` on ties, on insufficient evidence, and when the text contradicts its own winner; callers must treat `'unknown'` as "do not conceal".
- The veto measures over the text's **own** letters: URLs, @handles and **quotations** are scrubbed first (a page quoting Russian is not a Russian page), unless the quotation is half the text or more — then it _is_ the content and counts. The rung tallies still read the text whole; only the veto excludes quotations.
