...
- `@movar/page-content/youtube` — registers `YOUTUBE_EXTRACTOR`; covers BOTH generations of YouTube markup at once (the polymer→wiz "view-model" migration is live per-surface, verified 2026-08-08): classic desktop/mobile renderers (`ytd-video-renderer`, `ytd-grid-video-renderer`, `ytd-channel-renderer`, `ytm-video-with-context-renderer`, …) AND the migrated components — `yt-lockup-view-model` (watch-sidebar recommendation / search Mix/playlist card / migrated grid-cell body; `ytd-compact-video-renderer` and `ytd-radio-renderer` are gone from those surfaces) and `grid-shelf-view-model` holding `ytm-shorts-lockup-view-model` tiles (the Shorts shelf; `ytd-reel-shelf-renderer` is gone from /results — the `ytm-` prefix is served on desktop too). View-model cards have NO `[id="video-title"]`: their single `<h3>` is the only durable title anchor, so classic shapes carry `fallbackTextSelectors: ['h3']` consulted only when the classic anchors yield nothing (keeps classic samples byte-identical — a classic tile's h3 can hold badge chrome). Lockups classify on the `h3` ALONE with no fallback: their `yt-content-metadata-view-model` byline bakes the channel name together with UI-language view-count/date strings and the «Нове» badge (same contamination class as Google's ad location extension — pitfalls §1); fail open. Kind resolves per element (`kindFor`): a lockup with the stacked `yt-collection-thumbnail-view-model` is a `playlist`, a `grid-shelf-view-model` without shorts tiles is a generic `shelf`. Outermost-wins dedup collapses a lockup inside its `ytd-rich-item-renderer` grid cell (and rich items inside a `ytd-rich-shelf-renderer[is-shorts]`) onto the enclosing unit
...
- **Nested card dedup in Google AND YouTube**: both extractors post-filter to drop elements nested inside another selected element (outermost wins) — prevents sitelinks (child `[data-hveid]`) from doubling up with their parent result, and collapses a `yt-lockup-view-model` inside its `ytd-rich-item-renderer` grid cell (or rich items inside a `ytd-rich-shelf-renderer[is-shorts]`) onto the enclosing unit.
