...
function renderBlock(metrics: Metric[]): string {
  ...
  const note =
    ...
    'score is snapshotted by `pnpm metrics`. Badges flag the self-explanatory ' +
    'metrics — the list below is the full legend.</sub>';
...
function spliceBlock(readme: string, block: string): string {
  if (!readme.includes(METRICS_START) || !readme.includes(METRICS_END)) {
    throw new Error(
      '[readme-metrics] METRICS markers not found in README.md — restore the ' +
        '`<!-- METRICS:START … -->` / `<!-- METRICS:END -->` pair in the ## Metrics section.',
    );
  }
