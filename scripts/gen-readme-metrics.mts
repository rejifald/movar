...
const readmePath = nodePath.resolve(repoRoot, 'README.md');
const snapshotPath = nodePath.resolve(repoRoot, 'scripts/readme-metrics.snapshot.json');
const fallowMdPath = nodePath.resolve(repoRoot, '.metrics/fallow.md');
const bundleJsonPath = nodePath.resolve(repoRoot, '.metrics/content-bundle.json');
...
function aggregateCoverage(): { lines: number; branches: number } | null {
  ...
  for (const group of ['apps', 'packages'] as const) {
    for (const entry of readdirSync(nodePath.resolve(repoRoot, group), { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      const file = nodePath.resolve(repoRoot, group, entry.name, 'coverage/coverage-summary.json');
  ...
  if (snapshot.bundle) {
    metrics.push({
      ...
      description:
        ...
        'artifact is ~48 KB, gated at 56 KB by wxt.config.ts) — franc and the ' +
...
function spliceRegion(readme: string, start: string, end: string, block: string): string {
  ...
  const existing = new RegExp(String.raw`${escapeRegExp(start)}[\s\S]*?${escapeRegExp(end)}`);
